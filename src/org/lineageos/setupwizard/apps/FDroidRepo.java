...
import org.lineageos.setupwizard.util.IOUtils;
import org.lineageos.setupwizard.util.FDroidUtils;
...
import static org.lineageos.setupwizard.SetupWizardApp.FDROID_CATEGORY_DEFAULT_BACKEND;
...
public class FDroidRepo {
    ...
    @WorkerThread
    public static void loadFdroidJson(String category, String path, RecyclerView list,
            AppAdapter adapter) {
        try {
            File index = new File(path + "/index-v1.json");
            JSONObject obj = new JSONObject(new String(IOUtils.toByteArray(index)));
    ...
    @WorkerThread
    public static void addApp(String category, String path, JSONObject app, JSONObject packages,
            RecyclerView list, AppAdapter adapter) {
        try {
            ...
            if (app.has("localized")) {
                ...
                if (!TextUtils.isEmpty(value)) iconPath = path + "/" + packageName + "/en-US/"
                        + value;
            }
