...
import static org.lineageos.setupwizard.util.ManagedProvisioningUtils.GARLIC_LEVEL_SAFER;
import static org.lineageos.setupwizard.util.ManagedProvisioningUtils.GARLIC_LEVEL_SAFEST;
import static org.lineageos.setupwizard.util.ManagedProvisioningUtils.GARLIC_LEVEL_STANDARD;
...
public class GarlicLevelActivity extends BaseSetupWizardActivity {
    ...
    protected void onNextPressed() {
        super.onNextPressed();
        ManagedProvisioningUtils.setGarlicLevel(this, mSelection);
    }
    ...
    private void highlightView(View view) {
        ...
        for (int i = 0; i < layoutList.size(); i++) {
            if (view.getId() == layoutList.get(i)) {
                view.setBackground(
                    ContextCompat.getDrawable(this, R.drawable.garlic_level_background_focused)
                );
            } else {
                findViewById(layoutList.get(i)).setBackground(
                    ContextCompat.getDrawable(this, R.drawable.garlic_level_background)
                );
            }
        }
    }
